...
from tf_verify.read_net_file import *
from tf_verify.read_zonotope_file import read_zonotope
from tf_verify.ai_milp import *
from tf_verify.config import config
from tf_verify.constraint_utils import *
...
def mmain(verbose=True, dir_prefix = "./"):
    ...
    if zonotope_bool==False:
       assert dataset in ['mnist', 'cifar10', 'acasxu', 'fashion','selfmade','mnist_fixed'], "only mnist, cifar10, acasxu, and fashion datasets are supported"
    ...
    if is_saved_tf_model or is_pb_file:
        ...
        elif(dataset=='mnist' or dataset=='mnist_fixed'):
            num_pixels = 784
        elif (dataset=='cifar10'):
            num_pixels = 3072
        elif(dataset=='acasxu'):
    ...
    if dataset=='acasxu':
        ...
        lw = localwriter('OUTPUT_Deeppoly.txt', unique=True)
        print("/////// RESULTS STORED IN",lw.timeFile,"\\\\\\\\\\\\\\\ ")
        mstart = time.time()
        nubs = []
        nlbs = []
        imgs = []
        verified_flag = []
        tests = get_tests(dataset, config.geometric)
        lw.printT('Verification started at ' + str(time.time()) + '\n')
        for i, test in enumerate(tests):
            if config.from_test and i < config.from_test:
                continue
            ...
            if(label == int(test[0])):
                ...
                normalize(specUB, means, stds, dataset)
                start = time.time()
                perturbed_label, _, nlb, nub, _,_ = eran.analyze_box(specLB, specUB, domain, config.timeout_lp, config.timeout_milp, config.use_default_heuristic)
                ...
                if(perturbed_label==label):
                    verified_flag.append(0)
                    if verbose:
                        print("img", i, "Verified", label)
                    verified_images += 1
                else:
                    verified_flag.append(1)
                    if complete==True:
                        ...
                        if(verified_flag==True):
                            if verbose:
                                print("img", i, "Verified", label)
                            verified_images += 1
                            lw.printT('verified ' + str(i))
                        else:
                            if verbose:
                                print("img", i, "Failed")
                            cex_label,_,_,_,_,_ = eran.analyze_box(adv_image, adv_image, 'deepzono', config.timeout_lp, config.timeout_milp, config.use_default_heuristic)
                            if(cex_label!=label):
                        ...
                        if verbose:
                            print("img", i, "Failed")
                        lw.printT('not verified ' + str(i))
    
                correctly_classified_images +=1
                end = time.time()
                if verbose:
                    print(end - start, "seconds")
                lw.printT(' in ' + str(end-start) + '\n')
            else:
                verified_flag.append(2)
                if verbose:
                    print("img",i,"not considered, correct_label", int(test[0]), "classified label ", label)
                lw.printT(str(i) + ' not considered\n')
            nts = netname.split('/')[-1][:-3]
            np.save(dir_prefix + "_upper-bounds-" + str(epsilon) + "_" + str(lw.indx) + ".npy",
                    np.array(nubs))
            np.save(dir_prefix + "_lower-bounds-" + str(epsilon) + "_" + str(lw.indx) + ".npy",
                    np.array(nlbs))
        mende=time.time()
        i = 0
        #while os.path.isfile("/home/stefanie/Documents/thesis/framework/temp/"+nts+"_upper-bounds-" + str(epsilon) + "_" + str(i) + ".npy"):
        #    i += 1
        np.save(dir_prefix +"_images-" + str(epsilon) + "_" + str(i) + ".npy", np.array(imgs))
        np.save(dir_prefix +"_upper-bounds-" + str(epsilon) + "_" + str(i) + ".npy", np.array(nubs))
        np.save(dir_prefix +"_lower-bounds-" + str(epsilon) + "_" + str(i) + ".npy", np.array(nlbs))
        np.save(dir_prefix +"_verified-" + str(epsilon) + "_" + str(i) + ".npy", np.array(verified_flag))
        with open(dir_prefix + "_results-" + str(epsilon) + "_" + str(i) + ".txt", "w+") as f:
            f.write('analysis precision %f \n' %((verified_images+1)/(correctly_classified_images+1)))
            f.write('time %f s' %(mende-mstart))
        if verbose:
            print('analysis precision ',verified_images,'/ ', correctly_classified_images)
        ind = i
    dicti = dict()
    dicti['time']=mende-mstart
    dicti['verified']=verified_images
    dicti['total']=correctly_classified_images
    dicti['verified_list'] = dir_prefix +"_verified-" + str(epsilon) + "_" + str(i) + ".npy"
    print("/////// RESULTS STORED IN", lw.timeFile, "\\\\\\\\\\\\\\\ ")
    return ind, dicti
...
if __name__=='__main__':
    ...
    parser.add_argument('--domain', type=str, default=config.domain,
                        help='the domain name can be either deepzono, refinezono, deeppoly or refinepoly')
    ...
    parser.add_argument('--numproc', type=int, default=config.numproc,
                        help='number of processes for MILP / LP / k-ReLU')
    ...
    parser.add_argument('--dyn_krelu', action='store_true', default=config.dyn_krelu,
                        help='dynamically select parameter k')
